...
const classInfor = require('../jsonGroup filtered/A10.json');
const fs = require('fs');

const getDataBySheetId = async (sheet_id) => {
    try {
        const response = await axios.post('http://54.254.90.240:1880/teacher-sheet-A10', {
            sheet: sheet_id
        })
        let data
        try {
...
const getAllTeachData = async () => {
    try {
        ...
        const filePath = 'E:/TUNGTRAN/WORK/PLINK/ConvertTools/AxcelaTool/jsonResult/A10-teacher-part1.json';
        if (false) {
