...
const classInfor = require('../jsonGroup filtered/A7.json');
const fs = require('fs');

const getDataBySheetId = async (sheet_id) => {
    try {
        const response = await axios.post('http://54.254.90.240:1880/coach-sheet-A7', {
            sheet: sheet_id
        })
...
const getAllTeachData = async () => {
    try {
        ...
        const filePath = 'E:/TUNGTRAN/WORK/PLINK/ConvertTools/AxcelaTool/jsonResult/A7-coach-part1.json';
        if (false) {
