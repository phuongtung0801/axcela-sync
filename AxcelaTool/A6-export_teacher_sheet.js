...
const classInfor = require('../jsonGroup filtered/A6.json');
const fs = require('fs');

const getDataBySheetId = async (sheet_id) => {
    try {
        const response = await axios.post('http://54.254.90.240:1880/teacher-sheet-A6', {
            sheet: sheet_id
        })
        let data
        try {
...
const getAllTeachData = async () => {
    try {
        ...
        const filePath = 'E:/TUNGTRAN/WORK/PLINK/ConvertTools/AxcelaTool/jsonResult/A6-teacher-part1.json';
        if (false) {
