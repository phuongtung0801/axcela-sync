...
const classInfor = require('../jsonGroup filtered/A2.json');
const fs = require('fs');

const getDataBySheetId = async (sheet_id) => {
    try {
        const response = await axios.post('http://54.254.90.240:1880/coach-sheet-A2', {
            sheet: sheet_id
        })
...
const getAllTeachData = async () => {
    try {
        ...
        const filePath = 'E:/TUNGTRAN/WORK/PLINK/ConvertTools/AxcelaTool/jsonResult/A2-coach-part1.json';
        if (false) {
